...
function PlaceholderFrame({ ...props }) {
  return (
    <svg viewBox='0 0 366 729' aria-hidden='true' {...props}>
      ...
      />
      <rect x='154' y='29' width='56' height='5' rx='2.5' fill='#a4a4a4' />
    </svg>
  );
}
export interface PhoneFrameProps extends React.ClassAttributes<HTMLDivElement> {
  className?: string;
...
export function PhoneFrame({ className, children, ...props }: PhoneFrameProps) {
  return (
    <div className={clsx("relative aspect-[366/729]", className)} {...props}>
      ...
      <div className='absolute top-[calc(23/729*100%)] left-[calc(23/366*100%)] grid h-[calc(686/729*100%)] w-[calc(318/366*100%)] transform grid-cols-1 overflow-hidden bg-white pt-[calc(23/318*100%)]'>
        {children}
      </div>
      <PlaceholderFrame className='pointer-events-none absolute inset-0 h-full w-full fill-gray-900' />
      <img
        src={frame}
        alt='Phone Frame'
